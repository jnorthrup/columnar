 ...
 */
 fun < V> Map<Int, V>.sparseVect0r(): SparseVect0r<V> = run {
    @Suppress("UNCHECKED_CAST") val entries =
        ((this as? SortedMap<Int, V >)?.entries ?: entries.sortedBy { it.key }).toList()
 ...
 fun < V> bindSparse(
    ...
    k.size t2 if (driver.size <= 16) //64 byte cacheline
        ...
        k.toVect0r().binarySearch(x).takeUnless {
            0 > it
