        ...
        when (cqe.pointed.user_data) {
            1 ->
                if (cqe.pointed.res != -EFAULT && cqe.pointed.res != -ECANCELED) {
                    fprintf(
                        stderr, "write got %d, wanted -EFAULT "
                        "or -ECANCELED\n", cqe.pointed.res
                    )
                    goto err
                }
            ...
            ->
                if (cqe.pointed.res != -ECANCELED && cqe.pointed.res != -EINVAL) {
                    fprintf(
                        stderr, "Invalid link timeout got %d"
 ...
 */
 fun  test_single_link_timeout (ring:CPointer<io_uring>, unsigned nsec) :Int
    {
        ts:__kernel_timespec
        cqe:CPointer<io_uring_cqe>
        sqe:CPointer<io_uring_sqe>
 ...
 fun  test_timeout_link_chain5 (ring:CPointer<io_uring>) :Int
        ...
        when (cqe.pointed.user_data) {
            ...
            ->
                if (cqe.pointed.res && cqe.pointed.res != -ECANCELED) {
                    fprintf(
                        stderr, "Request got %d, wanted -EINVAL "
                        "or -ECANCELED\n",
                        cqe.pointed.res
                    )
                    goto err
    ...
    fun main(argc: Int, argv: CPointer<ByteVar>[]):Int
    {
        ...
        ret = test_single_link_timeout(ring.ptr, 100000UL L)
        if (ret) {
            printf("test_single_link_timeout 100000 failed\n")
            return ret
        }

        ret = test_single_link_timeout(ring.ptr, 500000000UL L)
        if (ret) {
