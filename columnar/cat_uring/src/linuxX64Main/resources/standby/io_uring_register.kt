...
static memfd_create:Int(name:String, flags:UInt)
{
...
int
expect_fail(fd:Int, opcode:UInt, arg:CPointer<ByteVar> ,
            nr_args:UInt, error:Int) {
    ret:Int;
...
int
new_io_uring(entries:Int, p:CPointer<io_uring_params>) {
    fd:Int;
...
map_filebacked(size:size_t) {
    ...
    addr = mmap(NULL, size,  PROT_READ or PROT_WRITE , MAP_SHARED, fd, 0);
    if (addr == MAP_FAILED) {
        perror("mmap");
        close(fd);
        return NULL;
...
long :ULongi nr_maps, nr_fds;
     ...
     */
    fd_as = mmap(NULL, UINT_MAX * sizeof(int),  PROT_READ or PROT_WRITE ,
    ...
    fds = mmap(fd_as, 128 * 1024 * 1024,  PROT_READ or PROT_WRITE ,
    ...
    for (i = 0; i < nr_maps; i++) {
        ...
        fds = mmap(fds, 128 * 1024 * 1024,  PROT_READ or PROT_WRITE ,
                    MAP_SHARED or MAP_FIXED , fdtable_fd, 0);
        if (fds == MAP_FAILED) {
...
test_iovec_size(fd:Int) {
    ...
    buf = mmap(NULL, 2 * pagesize,  PROT_READ or PROT_WRITE ,
                MAP_PRIVATE or MAP_ANONYMOUS , -1, 0);
    ...
    /* huge page */
    buf = mmap(NULL, 2 * 1024 * 1024,  PROT_READ or PROT_WRITE ,
...
void
dump_sqe(sqe:CPointer<io_uring_sqe>) {
    printf("\topcode: %d\n", sqe.pointed.opcode );
    printf("\tflags:  0x%.8x\n", sqe.pointed.flags );
    printf("\tfd:     %d\n", sqe.pointed.fd );
...
int
ioring_poll(ring:CPointer<io_uring>, fd:Int, int fixed) {
    ret:Int;
...
static test_shmem:Int(void) {
    ...
    mem = mmap(NULL, len,  PROT_READ or PROT_WRITE , MAP_SHARED, memfd, 0);
    if (!mem) {
