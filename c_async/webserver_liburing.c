    ...
    if (sock != -1) {
        ...
        if (setsockopt(sock,
            ...
            if (bind(sock, (const struct sockaddr *) &srv_addr, sizeof(srv_addr)) >= 0) {
                ...
                } else fatal_error("listen()");
            } else { fatal_error("bind()"); }
        } else fatal_error("setsockopt(SO_REUSEADDR)");
    } else fatal_error("socket()");
}

int add_accept_request(int server_socket, struct sockaddr_in *client_addr, socklen_t *client_addr_len) {
    struct io_uring_sqe *submission_queue_entry = io_uring_get_sqe(&the_ring_on_the_stack);
    io_uring_prep_accept(submission_queue_entry, server_socket, (struct sockaddr *) client_addr, client_addr_len, 0);
    struct request *req = malloc(sizeof(*req));
    req->event_type = EVENT_TYPE_ACCEPT;
    io_uring_sqe_set_data(submission_queue_entry, req);
    io_uring_submit(&the_ring_on_the_stack);
    return 0;
}
...
int add_read_request(int client_socket) {
    ...
    io_uring_prep_readv(submission_queue_entry, client_socket, &req->iov[0], 1, 0);
    io_uring_sqe_set_data(submission_queue_entry, req);
    io_uring_submit(&the_ring_on_the_stack);
    return 0;
}
...
int add_write_request(struct request *req) {
    ...
    io_uring_prep_writev(submission_queue_entry, req->client_socket, req->iov, req->iovec_count, 0);
    io_uring_sqe_set_data(submission_queue_entry, req);
    io_uring_submit(&the_ring_on_the_stack);
    return 0;
}
...
void server_loop(int server_socket_fd) {
    struct io_uring_cqe *completion_queue_entry_ptr;
    struct sockaddr_in client_addr_on_stack;
    socklen_t client_addr_len = sizeof(client_addr_on_stack);

    add_accept_request(server_socket_fd, &client_addr_on_stack, &client_addr_len);

    while (1) {
        int ret = io_uring_wait_cqe(&the_ring_on_the_stack, &completion_queue_entry_ptr);
        if (ret >= 0) {
            struct request *req_heap_ptr = selectorKeyAttachment(completion_queue_entry_ptr);
            ...
            if (0 <= resultCode) {
                switch (req_heap_ptr->event_type) {
                    ...
                    case EVENT_TYPE_WRITE:
                        ...
                        close(req_heap_ptr->client_socket);
                        free(req_heap_ptr);
                        break;
                }
...
void sigint_handler(int signo) {
    ...
    io_uring_queue_exit(&the_ring_on_the_stack);
    exit(0);
}
...
int main() {
    ...
    io_uring_queue_init(QUEUE_DEPTH, &the_ring_on_the_stack, 0);
